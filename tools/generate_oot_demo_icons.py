...
try:
    ...
    raise SystemExit(
        "This generator requires Pillow. Install with: python3 -m pip install pillow"
    ) from ex
...
def scale_points(
    points: Iterable[Tuple[float, float]], s: int
...
def badge(
    draw: ImageDraw.ImageDraw, size: int, fill: Color = rgba("#172351", 150)
) -> None:
    draw.rounded_rectangle(
        xy((6, 6, 58, 58), size),
        radius=stroke_width(size, 12),
        fill=fill,
        outline=rgba("#7a8ee8", 130),
        width=stroke_width(size, 1.2),
    )


def polygon(
    draw: ImageDraw.ImageDraw, size: int, points, fill, outline=DARK, width=1.6
):
    draw.polygon(pts(points, size), fill=fill)
    draw.line(
        pts(list(points) + [points[0]], size),
        fill=outline,
        width=stroke_width(size, width),
        joint="curve",
    )


def line(
    draw: ImageDraw.ImageDraw, size: int, points, fill=INK, width=3, joint="curve"
):
    draw.line(
        pts(points, size), fill=fill, width=stroke_width(size, width), joint=joint
    )


def ellipse(draw: ImageDraw.ImageDraw, size: int, box, fill, outline=DARK, width=1.5):
    draw.ellipse(
        xy(box, size), fill=fill, outline=outline, width=stroke_width(size, width)
    )


def rect(
    draw: ImageDraw.ImageDraw, size: int, box, fill, outline=DARK, width=1.5, radius=0
):
    draw.rounded_rectangle(
        xy(box, size),
        radius=stroke_width(size, radius),
        fill=fill,
        outline=outline,
        width=stroke_width(size, width),
    )
...
def draw_arrow_icon(size: int, direction: str) -> Image.Image:
    ...
    if direction == "left":
        polygon(
            draw,
            size,
            [(16, 32), (34, 14), (34, 25), (50, 25), (50, 39), (34, 39), (34, 50)],
            GOLD,
        )
    else:
        polygon(
            draw,
            size,
            [(48, 32), (30, 14), (30, 25), (14, 25), (14, 39), (30, 39), (30, 50)],
            GOLD,
        )
...
def draw_nut(size: int) -> Image.Image:
    ...
    ellipse(
        draw, size, (25, 28, 39, 42), rgba("#5f391e"), outline=rgba("#2c1b11"), width=1
    )
    return finish(img, size)
...
def draw_spell(size: int, color: Color, letter: str) -> Image.Image:
    ...
    draw.arc(
        xy((18, 18, 46, 46), size), 20, 330, fill=DARK, width=stroke_width(size, 3)
    )
...
def draw_ocarina(size: int) -> Image.Image:
    ...
    for x0, y0 in [(26, 31), (34, 32), (30, 39), (40, 39)]:
        ellipse(
            draw, size, (x0 - 2, y0 - 2, x0 + 2, y0 + 2), DARK, outline=DARK, width=0.5
        )
    return finish(img, size)
...
def draw_bombchu(size: int) -> Image.Image:
    ...
    polygon(
        draw,
        size,
        [(13, 39), (25, 24), (47, 23), (55, 36), (41, 45), (23, 48)],
        rgba("#4962b6"),
    )
...
def draw_boomerang(size: int) -> Image.Image:
    ...
    polygon(
        draw,
        size,
        [(13, 43), (31, 15), (51, 21), (40, 30), (32, 26), (23, 47)],
        rgba("#6fb7e9"),
        outline=WHITE,
    )
    return finish(img, size)
...
def draw_beans(size: int) -> Image.Image:
    ...
    for box, col in [
        ((17, 35, 34, 50), GREEN),
        ((30, 18, 48, 34), rgba("#89d455")),
        ((21, 18, 36, 35), rgba("#5baa38")),
    ]:
...
def draw_hammer(size: int) -> Image.Image:
    ...
    rect(
        draw, size, (25, 13, 53, 28), rgba("#6c7383"), outline=SILVER, width=2, radius=3
    )
    return finish(img, size)
...
def draw_poe(size: int) -> Image.Image:
    ...
    badge(draw, size)
    ellipse(
        draw, size, (17, 13, 47, 48), rgba("#8247c5", 210), outline=WHITE, width=1.5
    )
    polygon(
        draw,
        size,
        [(18, 40), (25, 55), (32, 42), (39, 55), (47, 40)],
        rgba("#8247c5", 210),
        outline=WHITE,
    )
...
def draw_scroll(size: int) -> Image.Image:
    ...
    badge(draw, size)
    rect(
        draw, size, (16, 18, 48, 46), rgba("#f1d48a"), outline=BROWN, width=2, radius=4
    )
...
def draw_mask(size: int) -> Image.Image:
    ...
    polygon(
        draw,
        size,
        [(16, 20), (32, 12), (48, 20), (45, 47), (32, 55), (19, 47)],
        rgba("#d79452"),
    )
...
def draw_boots(size: int, fill: Color, wing: bool = False) -> Image.Image:
    ...
    polygon(
        draw,
        size,
        [(18, 17), (29, 17), (30, 42), (42, 43), (45, 52), (18, 52)],
        fill,
        outline=DARK,
    )
    polygon(
        draw,
        size,
        [(36, 21), (47, 18), (47, 43), (54, 44), (56, 52), (36, 52)],
        fill,
        outline=DARK,
    )
    if wing:
...
def draw_player(
    size: int, tunic: Color = GREEN, cap: Color | None = None
) -> Image.Image:
    if cap is None:
        cap = tunic
    img, draw = icon_canvas(size)
    badge(draw, size, rgba("#112112", 130))
...
def draw_stone(size: int, color: Color, shape: str) -> Image.Image:
    ...
    if shape == "triangle":
        polygon(
            draw, size, [(32, 11), (51, 48), (13, 48)], color, outline=WHITE, width=2
        )
    elif shape == "ruby":
        polygon(
            draw,
            size,
            [(32, 9), (49, 24), (44, 50), (20, 50), (15, 24)],
            color,
            outline=WHITE,
            width=2,
        )
...
def draw_song(size: int, color: Color, mark: str) -> Image.Image:
    ...
    ellipse(draw, size, (18, 38, 28, 48), color, outline=WHITE, width=0.8)
    ellipse(draw, size, (34, 32, 44, 42), color, outline=WHITE, width=0.8)
...
def draw_hud_button(
    size: int, label: str, fill: Color, *, wide: bool = False
) -> Image.Image:
    img, draw = icon_canvas(size)
    ...
    if wide:
        ...
        radius = 24
    draw.rounded_rectangle(
        xy(shadow, size), radius=stroke_width(size, radius), fill=(0, 0, 0, 115)
    )
    draw.rounded_rectangle(
        xy(box, size),
        radius=stroke_width(size, radius),
        fill=fill,
        outline=rgba("#fff8ce"),
        width=stroke_width(size, 2.0),
    )
    r, g, b, a = fill
    highlight = (min(255, r + 42), min(255, g + 42), min(255, b + 42), a)
    draw.rounded_rectangle(
        xy(inner, size), radius=stroke_width(size, radius * 0.72), fill=highlight
    )
    draw.rounded_rectangle(
        xy((inner[0] + 1.5, inner[1] + 2.0, inner[2] - 1.5, inner[3] - 2.0), size),
        radius=stroke_width(size, radius * 0.62),
        fill=fill,
    )
    if label == "START":
        ...
        polygon(
            draw, size, [(19, 31), (29, 21), (29, 41)], WHITE, outline=BLACK, width=0.7
        )
    return finish(img, size)
